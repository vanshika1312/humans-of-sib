...
export default async function PersonPage({ params }: Props) {
  ...
  const person = await prisma.user.findUnique({
    ...
    include: {
      department: true,
      city: true,
      manager: { select: { id: true, name: true, image: true, title: true } },
      reports: { select: { id: true, name: true, image: true, title: true } },
    },
  });

  if (!person || person.status === "EXITED") notFound();

  const [wins, okrs, certs, journeyEvents] = await Promise.all([
    prisma.win.findMany({
      where: { userId: id },
  ...
  const tenure = Math.floor((Date.now() - person.joinedAt.getTime()) / (1000 * 60 * 60 * 24));
  const tenureLabel =
    tenure < 30
      ? `${tenure} days`
      : tenure < 365
      ...
      <Card className="overflow-hidden">
        ...
        <CardContent className="pt-0 -mt-10">
          <div className="flex items-end gap-4">
            ...
            />
            <div className="pb-1">
              <h1 className="text-xl font-bold text-ink-700">{person.name}</h1>
              <div className="text-sm text-ink-500">{person.title || "Team member"}</div>
              <div className="flex flex-wrap gap-1.5 mt-1.5">
                ...
                <Badge tone="orange">{person.role}</Badge>
              </div>
            </div>
          </div>

          <div className="grid md:grid-cols-3 gap-3 mt-5 pt-5 border-t border-ink-100">
            <InfoItem label="Joined" value={`${formatDate(person.joinedAt)} · ${tenureLabel}`} />
            <InfoItem label="Manager" value={person.manager?.name || "—"} />
            <InfoItem label="Team size" value={person.reports.length > 0 ? `${person.reports.length} direct reports` : "Individual contributor"} />
          </div>
